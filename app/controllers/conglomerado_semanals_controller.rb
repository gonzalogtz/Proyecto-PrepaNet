class ConglomeradoSemanalsController < ApplicationController
  ...
  def index
    if session[:rol] == STR_ROL_COORDINADOR_TUTOR
      @conglomerado_semanals = ConglomeradoSemanal.where(coordinador_tutores: session[:cuenta])
  ...
  private
    def verify_show_access(conglomerado_semanal)
      if (session[:rol] == STR_ROL_TUTOR && conglomerado_semanal.tutor == session[:cuenta])
        return true
      elsif (session[:rol] == STR_ROL_COORDINADOR_TUTOR && conglomerado_semanal.coordinador_tutores == session[:cuenta])
        return true
      elsif (session[:rol] == STR_ROL_COORDINADOR_CAMPUS && conglomerado_semanal.campus == session[:campus])
        return true
    ...
    def verify_edit_access(conglomerado_semanal)
      if (session[:rol] == STR_ROL_COORDINADOR_TUTOR && conglomerado_semanal.coordinador_tutores == session[:cuenta])
        return true
      elsif (session[:rol] == STR_ROL_COORDINADOR_CAMPUS && conglomerado_semanal.campus == session[:campus])
        return true
      elsif (session[:rol] == STR_ROL_COORDINADOR_PREPANET)
